...
export class Box extends React.Component {
    ...
    componentWillUnmount(): void {
        for (const page of this.props.pageMap.values()) {
            if (page.working) {
                page.stop()
            }
        }
    ...
    render() {
        return <div className={this.props.mainClass} ref={this.REFS.main} style={{display: 'none'}}>
            <div className={this.props.boxClass}>
                {(() => {
                    if ([...this.props.pageMap.values()].filter(page => page.isCurrent()).length !== 0)
                        return <Layout>
                            <Layout.Content>
                                {  // 页面
                                    [...this.props.pageMap.values()].filter(page => page.isCurrent()).map(page =>
